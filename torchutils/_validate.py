...
def _validate_param(val, name, param_type):
    ...
    if param_type not in supported_types:
        raise TypeError(('INTERNAL ERROR: param_type must be one of {}, but got {}. '
                    'Please raise an issue on github.').format(supported_types, type(val)))
    ...
    raise TypeError('{} must be of type {}, but got {}.'.format(name, type_str, type(val)))